...
function detectHermesRootHome(): string {
  return resolve(process.env.HERMES_HOME || join(homedir(), '.hermes'))
}
...
export class HermesSkillInjector {
  ...
  static resolveTargetDirs(rootDir = detectHermesRootHome()): string[] {
    ...
    try {
      ...
      // Single-profile installs may not have named profile directories.
    }
