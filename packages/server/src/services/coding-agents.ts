...
function buildCodexModelCatalog(input: {
  ...
  return {
    models: models.map((model, index) => codexCatalogEntry({
      ...
      // sunke's getModelContextLength is profile-keyed (resolves the profile's
      // default model via DB/config), not model-keyed like upstream's. The codex
      // catalog contextWindow is only a hint, so the profile's value is fine.
      contextWindow: getModelContextLength(input.profile),
      priority: index,
    })),
  }
}
