...
function safeRelativePath(value: string | null | undefined): string | null {
  ...
  if (!normalized || isAbsolute(raw) || normalized.startsWith('/') || /^[a-zA-Z]:/.test(normalized)) return null
  const parts = normalized.split('/').filter(Boolean)
  if (parts.includes('..')) return null
  return parts.join('/')
}

function safeWorkspaceLabel(value: string | null | undefined): string {
...
export function saveWorkspaceRunChange(change: SaveWorkspaceRunChangeInput): WorkspaceRunChangeSummary | null {
  ...
  const files = change.files
    .map(file => {
      ...
      if (!path) return null
      const oldPathValue = file.old_path || null
      const oldPath = safeRelativePath(oldPathValue)
      if (oldPathValue && !oldPath) return null
      return { ...file, path, old_path: oldPath }
    })
  ...
  const totalPatchBytes = files.reduce((sum, file) => sum + file.patch_bytes, 0)
  const truncated = Boolean(change.truncated || files.some(file => file.truncated))
  ...
  try {
    ...
    db.prepare(
      ...
      change.run_id || '',
      change.source || 'run',
      safeWorkspaceLabel(change.workspace),
...
export function listWorkspaceRunChangesForSession(sessionId: string): WorkspaceRunChangeSummary[] {
  ...
  const rows = db.prepare(
    `SELECT * FROM ${WORKSPACE_RUN_CHANGES_TABLE} WHERE session_id = ? ORDER BY created_at ASC, change_id ASC`,
