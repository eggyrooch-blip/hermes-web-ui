...
// ConnectorState vocab (hermes-multitenancy connectors/models.py): authenticated |
// configured | needs_auth | unknown | missing | error. Match EXACTLY — 'needs_auth'
// must not fall into the green 'ok' bucket via a naive substring check.
function credKind(s: string): 'ok' | 'warn' | 'mut' {
  if (s === 'authenticated' || s === 'configured') return 'ok'
