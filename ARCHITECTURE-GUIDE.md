...
> filesystem walk has a 1-second deadline, while the existing file/byte/depth
> caps and secret/symlink exclusions remain unchanged.
>
> `deleteSession()` now removes messages plus both workspace change tables in
> one SQLite transaction. A regression forces a message-delete failure and
> proves the session, message, summary, and patch rows all roll back together,
> then proves the successful delete leaves all four empty. Focused server
> verification is 5 files / 87 passed; the full suite is 318 files / 2481
> passed / 2 skipped, and production build plus `harness:check` pass. Evidence
...
- 2026-07-17：`webui-release-blockers` 将 workspace diff 的 Git、目录、文件与 patch 子进程改为异步 I/O；broker、bridge、coding-agent 均在 baseline 完成后才启动 run，并在 terminal event 前等待 diff，Git 5 秒与扫描 1 秒硬截止保留。`deleteSession()` 在同一 SQLite 事务内清理 message、session、workspace summary/file rows，并用强制删除失败验证整体回滚。Focused 5 files / 87 passed，full 318 files / 2481 passed / 2 skipped，production build 与 `harness:check` 通过。当前仅 ftask worktree，未 push、未合入 main、未发布生产。
