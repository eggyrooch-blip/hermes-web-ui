...
- `packages/client/src/stores/hermes/chat.ts` 必须用有界的 paginated HTTP fallback 恢复空 transcript，并用 request epoch + object identity merge 防止迟到响应覆盖发送中的 prompt/assistant state；空 reconnect payload 也不能清空已渲染消息。
...
- 2026-07-10：`webui-message-list-hotfix` 修复 upstream chat hydration 回归。Socket resume 为空、stale-zero 或 15 秒超时时，client 现在查询 paginated history；迟到页通过 per-session epoch 与对象身份合并保留新消息，空 reconnect 不再清 transcript，store dispose 清理 visibility listener/12 秒 interval。`messages.run_id` 从 schema、local store、bridge flush/resume 一路进入 paginated contract，恢复 inline diff 的 server-backed 匹配。回归为 focused client 81 passed、server 90 passed、full 313 files / 2372 passed / 2 skipped，`harness:check` 与 build passed；隔离 worktree `:8750` 使用真实本地 DB snapshot 的 Playwright 路径在 resume timeout 后发出 paginated `200`，渲染 7 条可见消息、空态 0。当前未合入本机 main，`:8648` 与生产均未部署/未验证。
