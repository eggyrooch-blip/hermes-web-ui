...
> Broker terminal handling completes and emits the old run's workspace diff
> before `markCompleted`, so a scheduled goal continuation cannot mutate the
> workspace before that diff is captured.
> Broker lifecycle cleanup is additionally scoped to the exact in-memory
> `SessionState` object plus its run marker. Session-generation mismatch uses a
> no-DB abandon callback that cancels the old queue without deleting a replacement
> state; setup rejection follows the same scoped cleanup. Abort only signals the
> active stream, whose sole finalizer runs after diff completion and dequeues from
> the latest queue, preventing an old completion from clearing a newly started run.
> Every incoming SSE frame is generation-fenced before it can mutate messages or
> emit into the session room. Goal continuations use the same FIFO as user work,
> yield to a queued user or abort, and only dequeue after the old terminal. Identity
> lookup and finalization-persistence failures emit a visible failure, release the
> exact run, and continue the latest queue instead of leaving `isWorking` stuck.
> Controller states also carry the SQLite rowid plus the process incarnation token.
> Public socket enqueue/start replaces a stale state after HTTP delete/recreate;
> every session create path uses the shared `run-chat/session-generation.ts`
> create-and-bind helper before another write or dispatch. Resume/replay hydration
> uses a generation CAS and cannot overwrite a run admitted while DB loading is
> pending; detecting a stale live generation immediately aborts and releases its
> old upstream instead of waiting for another SSE frame. Clean abort emits only
> `abort.completed`; goal evaluation has a dedicated linked abort controller, so
> either a user queue or an explicit abort can stop a hanging evaluation without
> aborting the next stream, while an abort cleanup
> persistence error marks `abort.completed` as failure-pending so client handlers
> remain attached for the following concrete failure. Usage emits are owner-
> guarded, and the unreachable `hermesSessionIds/syncFromHermes` late writer was removed.
> `/plan` and `/goal` share the user FIFO. Once dequeued, their broker command lookup
> holds an exact state/marker/generation reservation through kickoff; abort or
> same-id recreation invalidates it before any command message/event/hidden run.
> Command generation mismatch abandons the exact mapped stale state (including an
> active nonserialized-command sibling), while a one-time post-create generation
> read failure rebinds the new row before surfacing the setup error so queued work
...
> This includes setup exceptions reached while another socket already owns the
> session: the room receives no terminal event and the active run keeps streaming.
...
> results. The submitted slash-command row reuses its optimistic `queue_id` as
> `messages.client_id`; each persisted result gets its own stable `client_id`, and
> the matching `session.command` carries it as `command_message_id`. On resume the
> client enriches that already hydrated row instead of appending a duplicate.
> If generation lookup itself fails, the command failure is still forced into the
> pending replay set even when a queued run makes it `terminal:false`; because no
> result row can be persisted safely, the pending `resume_event_id` also becomes
> `command_message_id` until each socket acknowledges the exact event.
...
> `resume.events.ack`.
> HTTP single and batch deletion abandon the exact row/incarnation generation, then
> transactionally delete local session/message/patch state before awaiting remote
> Hermes cleanup. The broker matches that generation, and the
> corresponding generic chat runtime is synchronously detached server-side before the
> remote await, so correctness does not depend on a later browser socket abort and a
> newly admitted run cannot be erased or resurrect the deleted local generation.
> Bridge error handling likewise retains exact run ownership until workspace diff
> completion and all usage/context awaits settle; only then does it release state,
> read the current queue, emit a visible failure (including finalization errors), and
> dequeue once.
...
> goal/user/abort FIFO, new user/plan bootstrap, real SQLite delete/recreate,
...
>
> 状态仅保存在已挂载客户端组件内，不新增 localStorage、schema 或服务端 API。workspace key 同时包含 runtime mode、profile metadata 和 session id，避免相同 session id 在不同运行面/profile 间串 tab；`FilePreview`、`ArtifactBrowser`、`FilesPanel` 继续复用，HTML 仍走既有 sandboxed artifact browser，diff/文本/Markdown/图片仍走现有 preview/editor。异步 entries/preview/editor 读取使用 request epoch 丢弃过期返回；files store 的编辑缓冲按 scope 分开保存，因此不同 session/profile 可同时保留未保存内容。任意 scope 删除或改名时会让所有受影响缓冲失效，避免稍后保存把已删除文件重新写回。secondary browser 仍在 scope 切换时重置。
>
> 无障碍边界包括 `tablist/tab/tabpanel`、`aria-selected/controls`、roving tabindex、方向键与 Home/End，以及关闭/浏览按钮的可读 label。普通用户没有 super-admin 文件/终端开关时，产物 header 自带 `Collapse`，关闭只隐藏 panel、不销毁 tab；聊天 workspace 图片卡现在把 PNG/JPG/JPEG/GIF/SVG/WebP/BMP/ICO 交给既有 `FilePreview`，非 workspace 图片也不会误走文本读取，其它不支持格式仍下载。独立文件页保留双击编辑，聊天 secondary browser 的双击则打开预览 tab。安全边界不变：浏览器不获得 broker bearer，ArtifactBrowser 可见地址不含 token，iframe sandbox 属性不放宽，文件访问仍由现有 BFF、chat-plane access control 与 profile workspace containment 约束。当前仅在本机 worktree；全量 Vitest、focused 71/71、Playwright 6/6、`npm run typecheck`、`npm run harness:check`、`npm run build` 与真实 Chrome 桌面/720px/HTML/Markdown/GIF 截图均已完成；真实三帧 GIF 相隔 800ms 显示不同帧。Claude Opus 正式异模型复审逐项确认 7 个 finding 已修复，`VERDICT: PASS`、`QA_VERDICT: PASS`。未合入 main，未 push，未发布生产。
...
> [!info] 2026-06-24 本机 worktree — ProfileSelector 共享入口补齐，生产未发布
> `webui-share-selector-entry` fixes the real ordinary-user entry point for agent sharing. The original `agent-sharing-acl` UI exposed member management only from `/hermes/profiles` `ProfileCard.vue`, but that route and sidebar item are super-admin-only, so Feishu users validating from the lower-left ProfileSelector could not see sharing. ProfileSelector now shows `profiles.share.manage` beside owner agent profiles and manager-shared agent profiles; clicking it uses the same `/api/hermes/agents/:agentId/shares` BFF list/grant/revoke flow. viewer/editor shared profiles keep the role badge but do not show share management, and shared profile metadata/avatar editing remains hidden.
>
...
>
> The browser never sends actor principal fields. Server routes under `/api/hermes/agents/*/shares` derive Run Broker identity headers from the verified `WebUser`: `X-Hermes-Actor-Provider`, `Tenant-Key`, `App-Id`, `User-Id`, `Display-Name`, `Avatar-Url`, and `Email`. Header values must be ByteString-safe before reaching `fetch`; non-ASCII actor display names are sent as `X-Hermes-Actor-Display-Name-Encoded=encodeURIComponent(name)` and decoded by the broker. Feishu OAuth session cookies now persist `userId`, `unionId`, `tenantKey`, `appId`, and `email`; `exchangeFeishuCode()` fetches `user_info` when token data lacks canonical principal fields even if name/avatar are already present. Legacy cookies created before those fields existed are still accepted: if the verified user's bound profile is a personal `feishu_<user_id>` profile, the BFF derives `X-Hermes-Actor-User-Id` from that profile suffix and uses the Feishu app id as a legacy tenant key. That app-id tenant fallback is only for profile-derived legacy actors; a session with explicit `userId` but no `tenantKey` fails closed and does not emit actor principal headers. This keeps multitenancy ownership tied to the authenticated Feishu session instead of request body data.
>
...
### §8.9 chat session resume 不是唯一消息事实源
...
- 2026-07-24：本机候选 `cloud-agent-session-stability` 选择性吸收 upstream #2203/#2207 的云端稳定性语义，不引入 Electron/Desktop Browser。客户端 abort lifecycle 改为 session-keyed Map；session-list full load 与 metadata refresh 共用 request epoch；A→B→A resume 使用 per-session latest epoch，并在分页 hydration/workspace diff 的异步写入前校验 epoch + exact `Session` identity。`VirtualMessageList` 同时观察 viewport 与 rendered content，高度延迟增长时仅为仍在跟底的用户保持贴底。focused 3 files / 94 tests 与 client typecheck 通过；当前仅 ftask worktree，未合入、未发布生产。
