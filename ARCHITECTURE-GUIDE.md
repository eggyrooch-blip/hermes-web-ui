...
>
> 当前已通过旧版 ftask ship 合入本机 `main`，push/finalize/worktree 删除等待 sunke 在 main checkout 验证；追加反馈仍在 task worktree，未重新 ship，未 push GitHub，未发布生产。追加反馈后的本机验证：focused Vitest 8 files / 81 tests passed；Playwright mocked e2e `tests/e2e/user-connectors-surface.spec.ts` 5 passed；`npm run build` passed；还需重新 review/ship、重启本机 `8648` 后再让 sunke 验证。
