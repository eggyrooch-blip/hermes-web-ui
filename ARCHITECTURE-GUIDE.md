...
>
> Follow-up `expert-sidebar-active-avatar`: the active chat sidebar row reuses
> the same active expert avatar for its agent logo, but only while that row is
> active and not a coding-agent session. Inactive/history rows remain Hermes;
> Codex/Claude rows keep their per-agent logos. This intentionally avoids
> pretending old sessions know which expert was selected before session-level
> expert metadata is persisted.
