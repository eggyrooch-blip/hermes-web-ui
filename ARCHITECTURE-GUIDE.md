...
>
> BFF 返回的头像类型新增 `avatar.type="url"` 与 `source="feishu_user|feishu_group"`；前端 `ProfileAvatar` 同时支持 data URL 和远端 URL，远端图片加载失败会回退 generated avatar，ProfileSelector 不改变切换前端 profile 的语义。Open Platform token 与群头像结果做短 TTL 内存缓存；日志只记录 profile 与错误摘要，不记录 token、secret 或 Open Platform 原始响应。本轮仅本机 worktree 验证，尚未合入本机 main，未 push，未发布生产。验证：focused Vitest `profile-avatar/profile-selector/profiles-routes` 为 30 passed；全量 `npm test` 为 279 files / 2055 passed / 2 skipped；`npm run build` passed。
