...
>
> Credentials 页 grid `align-items: start` 与 required skill 滚动区保留，避免 Lark-cli/kep-cli 关联技能列表撑高同排卡片。`meegle-credential-card-fix` 追加修正：没有全局 `meegle` 但 PATH 有 `npx` 时，卡片不再显示“未安装”，点击授权才走 `npx -y @lark-project/meegle`；状态页不会主动跑 npx 以免 npm 下载拖慢。`meegle-launchd-npx-path` 再补 launchd 窄 PATH 场景：即使服务进程 PATH 只有 `/usr/bin:/bin:/usr/sbin:/sbin`，也会扫描 `HERMES_MEEGLE_EXTRA_PATHS`、`/opt/homebrew/bin`、`/usr/local/bin` 来定位 `meegle/npx`。`meegle-node-path-for-npx` 进一步保证执行 Meegle/npx 子进程时 PATH 也包含这些目录，避免 Homebrew `npx` 的 `#!/usr/bin/env node` 在 launchd 环境下找不到 `node`。飞书项目 `required_by` 只认明确 Meegle/Meego/飞书项目/project.feishu.cn 信号，不再把含“需求/工作项/排期”的 kep/lark 技能误挂到飞书项目。状态响应不包含 access/refresh token 或 keychain 内容。
