...
>
> Owner agent profiles with `agentId` get a compact sharing modal in `ProfileCard.vue` for list/grant/revoke. Roles are `viewer`, `editor`, and `manager`: viewer can run and read only chat-plane-safe config sections; editor can additionally write only `display`, `session_reset`, and `privacy`; manager can manage members and see all WebUI sessions for the shared agent. Model/provider/credentials/auxiliary config, profile metadata, avatar editing, and raw owner secrets remain outside the shared surface. Verification for this worktree: full Vitest 285 files / 2080 passed / 2 skipped, `vue-tsc -b` passed, server `tsc --noEmit` passed, `npm run build` passed.
