...
>
> 2026-05-20 16:xx 追加群聊 agent 模型可见性与 profile 默认模型：群聊消息 sender、成员 popover 和“添加智能体”profile 下拉现在用 `/api/hermes/profiles` 已有的 `model` metadata 渲染 `agent/profile · model`，不新增群聊私有状态；新建 profile 弹窗复用 upstream 模型接口 `/api/hermes/available-models`，默认选中当前/默认模型，并在 profile 创建成功后仅写新 profile 自己的 `config.yaml model.default/provider`。Hermes CLI/core 不新增参数，仍先走 upstream `hermes profile create`，模型写入是 WebUI 兼容层。验证：focused profile/group/server tests 4 files / 16 passed，`pnpm run build` passed；本机 launchd `com.hermes.ekko-webui-upstream-0530` 已切回 `/Users/kite/code/hermes-web-ui.tasks/webui-upstream-0530/dist/server/index.js`，`/health` OK。生产未发布。
