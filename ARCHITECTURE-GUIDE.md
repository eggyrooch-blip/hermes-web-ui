...
> `sidebar-enterprise-chrome` 已在本机合入 main；运行代码变更截止 `1849ddd8`，随后只有 docs-only 收尾，并已重建本机 launchd `com.hermes.ekko-webui` 的 `dist/server/index.js`；生产尚未发布，GitHub 尚未 push/finalize。边界是 WebUI 适配层，不改 Hermes Agent，不把员工运维/升级入口暴露到普通用户。
>
