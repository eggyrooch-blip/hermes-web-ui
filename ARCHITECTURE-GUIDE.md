...
- `packages/client/src/stores/hermes/chat.ts` 必须用有界的 paginated HTTP fallback 恢复空 transcript，并用 request epoch + serialized snapshot merge 防止迟到响应覆盖发送中的 prompt/assistant state；空 reconnect payload 也不能清空已渲染消息。乐观 user row 与数字 server echo 只允许一对一对账，不能重复显示，也不能吞掉短时间内真正重复发送的 prompt。
