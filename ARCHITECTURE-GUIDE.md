...
> `webui-expert-automation-nav` 调整 Feishu/WebUI 普通用户登录后的主界面产品入口：chat/home 窄侧栏顶部先显示同源 `/logo.png` + `Hermes` 品牌入口，位于 `新建对话` 上方；随后直接显示 `新建对话 / 搜索 / 专家 / 自动化 / 历史`，`专家` 位于 `自动化` 上方，`自动化` 位于 `历史` 上方；点击 `专家` 不再跳离 chat 壳，而是进入 `/#/hermes/chat?surface=expert` 并在右侧主内容区内嵌 ExpertView；点击 `自动化` 进入 `/#/hermes/chat?surface=automation` 并在同一主内容区内嵌 JobsView。`/hermes/expert` 与 `/hermes/jobs` 独立路由仍保留给直达兼容，不作为 chat 侧栏的跳转目标；非 chat 的 AppSidebar 不再渲染 `专家/自动化`，恢复 `文件 / 看板 / 记忆` 等页面入口，其中 `文件` 指向既有 `/#/hermes/files` 文件浏览页，顶部品牌链接同样复用同源静态资源 `/logo.png` 并保留 `Hermes` 文案。chat 窄侧栏底部原独立“设置”按钮位置改为飞书用户卡片，显示姓名、profile、连接状态，设置入口变成卡片内齿轮 icon；该卡片与 AppSidebar 复用 `SidebarUserCard`。`专家` 图标改成人像徽章；原 Jobs 产品文案更名为 Automation/自动化，但保留 `/hermes/jobs` route、store 和后端 API 名称，避免扩大后端迁移面。
>
