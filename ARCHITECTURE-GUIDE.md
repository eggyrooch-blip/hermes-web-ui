...
>
> 无障碍边界包括 `tablist/tab/tabpanel`、`aria-selected/controls`、roving tabindex、方向键与 Home/End，以及关闭/浏览按钮的可读 label。普通用户没有 super-admin 文件/终端开关时，产物 header 自带 `Collapse`，关闭只隐藏 panel、不销毁 tab；聊天 workspace 图片卡现在把 PNG/JPG/JPEG/GIF/SVG/WebP/BMP/ICO 交给既有 `FilePreview`，其它不支持格式仍下载。安全边界不变：浏览器不获得 broker bearer，ArtifactBrowser 可见地址不含 token，iframe sandbox 属性不放宽，文件访问仍由现有 BFF、chat-plane access control 与 profile workspace containment 约束。当前仅在本机 worktree；全量 317 files / 2463 passed / 2 skipped、Playwright 6/6、`npm run typecheck`、`npm run harness:check`、`npm run build` 与真实 Chrome 桌面/720px/HTML/Markdown/GIF 截图均已完成；真实三帧 GIF 相隔 800ms 显示不同帧。未合入 main，未 push，未发布生产。
