...
> WebUI 的飞书 OAuth 登录仍只负责 `open_id -> profile` 身份绑定，不把 OAuth access token 当工具 UAT 使用，也不把 UAT 存入 cookie/localStorage/WebUI DB。登录页拿到有效 `hermes_feishu_session` 后，会先调用受保护 BFF 接口 `GET /api/auth/feishu/uat/status`；若 multitenancy credential vault 中当前 `profile + open_id` 缺少有效 UAT 或 scope 不足，再调用 `POST /api/auth/feishu/uat/start`，展示等价于 `/feishu_auth` 的 device-flow 授权链接和 user code，并轮询 `/api/auth/feishu/uat/sessions/:sessionId`。路由守卫也会在 Feishu OAuth 模式进入受保护页面前检查 UAT，避免已有浏览器 session 直接停在 `/hermes/chat` 时绕过授权页。
>
...
>
> 生产修正：`1c76058` 显式在 chat plane 放行 `/api/auth/feishu/uat/*`。这些接口仍位于 `authProtectedRoutes` 之后，必须先有 WebUI session；放行只避免 `/api/auth/*` 管理面 denylist 误挡用户自助授权。Chrome 实测清空 `sunke` UAT 后，`/api/auth/feishu/uat/status` 返回 `status=missing`，进入 `/#/hermes/chat` 展示“授权飞书工具”和 device-flow user code。
...
- 2026-05-15：新增 WebUI Feishu UAT ensure：受保护 BFF 接口 `/api/auth/feishu/uat/{status,start,sessions/:id}` 通过服务器端 session 身份代理到 multitenancy Run Broker sidecar；LoginView 和前端路由守卫在进入 chat 前先验证 UAT，不足时展示 device-flow 授权链接，不接触原始 UAT token。`1c76058` 追加修复 chat-plane access control，允许这些已登录用户自助授权接口通过，不开放其他 `/api/auth/*` 管理面。
