...
- `packages/client/src/stores/hermes/chat.ts` 必须用有界的 paginated HTTP fallback 恢复空 transcript，并用 request epoch + serialized snapshot merge 防止迟到响应覆盖发送中的 prompt/assistant state；空 reconnect payload 也不能清空已渲染消息。乐观 user/command row 的 `queue_id` 必须以 `messages.client_id` 持久化并经 resume/paginated 返回，客户端只按稳定 ID 对账，不能用内容或时间戳猜测，否则会重复显示或吞掉真正重复发送的 prompt。
...
- 2026-07-10：`webui-message-list-hotfix` 修复 upstream chat hydration 回归。Socket resume 为空、stale-zero 或 15 秒超时时，client 查询 paginated history；迟到页通过 per-session epoch 与 serialized snapshot merge 保留新消息，空 reconnect 不清 transcript。user/command `queue_id` 以 `messages.client_id` 贯通本地存储、resume 与 paginated，恢复候选只认稳定 client identity 后的 server order 或明确 run marker，不再按内容/时间戳猜测。`messages.run_id` 贯通 Responses、Run Broker、coding-agent、bridge flush/resume 与 paginated contract。回归为 focused client 91 passed、server 167 passed、full 313 files / 2386 passed / 2 skipped，`harness:check` 与 build passed；隔离 worktree `:8750` 用真实 DB snapshot 在 resume timeout 后得到 paginated `200`、7 条可见消息、空态 0。历史页 10 条均有 `run_id` 字段但非空值为 0，因此浏览器只证明 hydration/shape，新写 identity 由测试证明。初版 fallback 已在本机 main `acda7568`，本轮加固仍只在 worktree；生产未部署/未验证。
