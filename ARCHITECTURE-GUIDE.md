...
> `sidebar-enterprise-chrome` 已在本机合入 main；运行代码变更截止 `506345f9`（继 `e63f71de` profile 列表快路径后，补员工态 frontend profile 与聊天 session 对齐），并会重建本机 launchd `com.hermes.ekko-webui` 的 `dist/server/index.js`；生产尚未发布，GitHub 尚未 push/finalize。边界是 WebUI 适配层，不改 Hermes Agent，不把员工运维/升级入口暴露到普通用户。
>
